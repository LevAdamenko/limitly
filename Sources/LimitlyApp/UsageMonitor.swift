...
final class UsageMonitor: ObservableObject {
    ...
    func weeklyText(for agent: AgentID) -> String { guard let usage = snapshot.weeklyUsage[agent] else { return "No data" }; let pct = settings.weeklyBudget(for: agent).percentage(for: usage).map { " (\(Int($0.rounded()))%)" } ?? ""; return format(usage, unit: settings.budget(for: agent).unit) + pct }
    ...
    private func apply(_ result: UsageSnapshot) {
        ...
        let weeklyPercentages = weeklyPercentages(result.weeklyUsage)
    ...
    private func weeklyPercentages(_ usage: [AgentID: UsageTotals]) -> [AgentID: Double] { Dictionary(uniqueKeysWithValues: usage.compactMap { agent, totals in settings.weeklyBudget(for: agent).percentage(for: totals).map { (agent, $0) } }) }
...
private enum CCUsageClient {
    static func fetch(now: Date = Date()) throws -> UsageSnapshot {
        ...
        return UsageSnapshot(currentUsage: currentUsage, weeklyUsage: parser.aggregate(rows), resetTimes: resetTimes)
    }
